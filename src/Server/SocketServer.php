...
class SocketServer
{
    ...
    public function expose(string $endpoint): void
    {
        ...
        if (isset($this->addresses[$name])) {
            throw new \Exception("Already listening to {$name} on HTTP server");
        }
    ...
    public function serve(): void
    {
        ...
        try {
            ...
            foreach ($this->servers as $server) {
                ...
                Log::channel('http')->info("Listening on {$scheme}://{$serverName}/");
            ...
            Log::alert("Failed to start socket server: {message}", [
